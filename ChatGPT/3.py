...
# 1. График x от t для всех формулировок
...
plt.savefig('disp2time_comparison.png')
plt.show()

# 2. Зависимость dx/dt от t для всех формулировок
plt.figure(figsize=(10, 6))
plt.plot(t.cpu().detach().numpy(), dxdt_1.cpu().detach().numpy(), label='Formulation 1')
plt.plot(t.cpu().detach().numpy(), dxdt_2.cpu().detach().numpy(), label='Formulation 2')
plt.plot(t.cpu().detach().numpy(), dxdt_3.cpu().detach().numpy(), label='Formulation 3')
plt.legend()
plt.xlabel('Time (t)')
plt.ylabel('Velocity (dx/dt)')
plt.title('Comparison of different formulations for dx/dt(t)')
plt.savefig('dxdt_comparison.png')
plt.show()

# 3. Зависимость d2x/dt2 от t для всех формулировок
plt.figure(figsize=(10, 6))
plt.plot(t.cpu().detach().numpy(), d2xdt2_1.cpu().detach().numpy(), label='Formulation 1')
plt.plot(t.cpu().detach().numpy(), d2xdt2_2.cpu().detach().numpy(), label='Formulation 2')
plt.plot(t.cpu().detach().numpy(), d2xdt2_3.cpu().detach().numpy(), label='Formulation 3')
plt.legend()
plt.xlabel('Time (t)')
plt.ylabel('Acceleration (d2x/dt2)')
plt.title('Comparison of different formulations for d2x/dt2(t)')
plt.savefig('d2xdt2_comparison.png')
plt.show()
